...
from ann_benchmarks.constants import LOCAL_RESULT_DIR
...
def get_result_filename(dataset, count, definition, query_arguments):
  d = [
    ...
    re.sub(r'\W+', '_', json.dumps(definition.arguments + query_arguments, sort_keys=True)).strip(
      '_'
    ),
  ]
...
def list_algorithms(definition_file):
  ...
  logger.info('The following algorithms are supported...')
  for point in definitions:
    logger.info('\t... for the point type "%s"...' % point)
    for metric in definitions[point]:
      logger.info('\t\t... and the distance metric "%s":' % metric)
...
def get_definitions(
  definition_file, dimension, point_type="float", distance_metric="euclidean", count=10
):
  definitions = _get_definitions(definition_file)
