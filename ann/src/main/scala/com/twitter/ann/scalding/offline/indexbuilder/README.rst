...
- **output_dir** Where to save the produced serialized ann index. Save to HDFS by specifying the full URI. e.g `hdfs://hadoop-dw2-nn.smf1.twitter.com/user/mhansmire/index_file` or using the default cluster `hdfs:///user/mhansmire/index_file`.
...
.. code-block:: bash
  ...
  packer add_version --cluster=smf1 <role> <package_name> dist/indexbuilder-deploy.zip
