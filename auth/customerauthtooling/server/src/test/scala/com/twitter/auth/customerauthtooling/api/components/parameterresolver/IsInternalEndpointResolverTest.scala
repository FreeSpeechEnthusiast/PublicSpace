...
class IsInternalEndpointResolverTest
    extends AnyFunSuite
  ...
  test("test IsInternalEndpointChecker with unknown endpoint") {
  ...
  test("test IsInternalEndpointChecker with internal endpoint") {
    when(routeInformationService.checkEndpoint(EndpointInfo(url = testUrl)))
      .thenReturn(
        Future.value(
          Some(
    ...
    Await.result(resolver.checkWithOverride(EndpointInfo(url = testUrl))) mustBe Some(
      AdoptionParameterValue[AdoptionParameter.IsInternalEndpoint](underlying = true))
  }

  test("test IsInternalEndpointChecker with public endpoint") {
    when(routeInformationService.checkEndpoint(EndpointInfo(url = testUrl)))
      .thenReturn(
        Future.value(
          Some(
            RouteInformation(
              ...
              cluster = "test"))))
    Await.result(resolver.checkWithOverride(EndpointInfo(url = testUrl))) mustBe Some(
      AdoptionParameterValue[AdoptionParameter.IsInternalEndpoint](underlying = false))
  }
