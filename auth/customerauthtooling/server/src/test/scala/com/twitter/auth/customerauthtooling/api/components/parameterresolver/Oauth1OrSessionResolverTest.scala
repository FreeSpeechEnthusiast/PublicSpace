...
class Oauth1OrSessionResolverTest
    extends AnyFunSuite
  ...
  private val resolver = Oauth1OrSessionResolver(routeInformationService = routeInformationService)

  private val testUrl = "/endpoint"

  test("test Oauth1OrSessionChecker with unknown endpoint") {
  ...
  test("test Oauth1OrSessionChecker with oauth2 auth endpoint") {
    when(routeInformationService.checkEndpoint(EndpointInfo(url = testUrl)))
      .thenReturn(
        Future.value(
          Some(
    ...
    Await.result(resolver.checkWithOverride(EndpointInfo(url = testUrl))) mustBe Some(
      AdoptionParameterValue[AdoptionParameter.Oauth1OrSession](underlying = false))
  }

  test("test Oauth1OrSessionChecker with session auth endpoint") {
    when(routeInformationService.checkEndpoint(EndpointInfo(url = testUrl)))
      .thenReturn(
        Future.value(
          Some(
            RouteInformation(
              ...
              authTypes = List(TAuthenticationType.Session).map(RouteAuthType.fromThrift),
              cluster = "test"))))
    Await.result(resolver.checkWithOverride(EndpointInfo(url = testUrl))) mustBe Some(
      AdoptionParameterValue[AdoptionParameter.Oauth1OrSession](underlying = true))
  }
